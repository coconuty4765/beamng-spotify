...
async fn login() -> impl Responder {
    ...
    let redirect_uri = String::from("http://127.0.0.1:8888/api/v1/callback");
...
async fn callback(info: web::Query<AuthRequest>) -> impl Responder {
    ...
    let token_request = Client::new()
        ...
        .form(&[
            ...
            ("code", &code),
            ("redirect_uri", "http://127.0.0.1:8888/api/v1/callback"),
        ]);
...
");
    ...
    if !Path::new("refresh_token.txt").exists() {
        ...
        let _ = open::that("http://127.0.0.1:8888/login");
    }
    ...
    }).workers(2).bind("127.0.0.1:8888").unwrap_or_else(|e| {
        panic!("Failed to bind to 127.0.0.1:8888: {}", e.to_string())
